...
**フロー:**
1. AIがHTMLを読み込み、セクション数をカウント（ヘッダーナビ・フッター除外）
2. セクション数をユーザーに伝える
3. ユーザーがその数に合わせて、上から順にセクション名を回答（ヘッダーナビ・フッター除く）
4. フルワイド（画面幅いっぱい）や少し広めに表示したいセクションがあるか確認
5. 必要に応じてユーザーがコードを確認して回答

**例（01_ホームページの場合）:**
```
AI: 「01_ホーム」ページのセクション構成を教えてください。
    HTML内に 5個のセクション が確認されています（ヘッダーナビ・フッターを除く）。
    ヘッダーナビとフッターを除いて、上から順に5個のセクション名を教えてください。

User: 上から、ヒーローセクション、サービス特徴A、サービス特徴B、料金表、お問い合わせ

AI: フルワイド（画面幅いっぱい）や少し広めに表示したいセクションはありますか？
    - フルワイドにしたい場合: セクション名を教えてください
    - 少し広めにしたい場合: セクション名を教えてください
    - 特になければ「なし」と回答してください

User: ヒーローセクションをフルワイドで

AI: 了解しました。以下のID名と幅指定で設定します:
  1. hero-section (フルワイド)
...
AI: 「02_料金」ページのセクション構成を教えてください。
    HTML内に 3個のセクション が確認されています（ヘッダーナビ・フッターを除く）...
```

**注意:**
- ヘッダーナビとフッターはID付与対象外（自動でタグ化される）
...
【01_ホーム】: hero-section (フルワイド), service-features-a, service-features-b, pricing-table, contact
【02_料金】: pricing-hero, plan-comparison (少し広め), faq
... (以下略)

※ヘッダーナビ・フッターは自動でタグ化されるため、上記には含まれていません
...
#### 実行例（1ページずつ）
```bash
# 01_ホームページ（ヘッダーナビ・フッター除く、幅指定あり）
.venv/Scripts/python.exe docs/helper/setup_relume_project.py \
  --page-dir "s:/MyProjects/KAMUI_CODE/Relume/2025-10-08_satokupo-design-test/copy-of-ホームページ作成サービス/01_ホーム" \
  --sections "hero-section:full-wide,service-features,pricing-table,contact"

# 02_料金ページ（ヘッダーナビ・フッター除く、幅指定あり）
.venv/Scripts/python.exe docs/helper/setup_relume_project.py \
  --page-dir "s:/MyProjects/KAMUI_CODE/Relume/2025-10-08_satokupo-design-test/copy-of-ホームページ作成サービス/02_料金" \
  --sections "pricing-hero,plan-comparison:wide,faq"
```

**幅指定の形式:**
- `セクション名:full-wide` → フルワイド（画面幅いっぱい）
- `セクション名:wide` → 少し広め
- `セクション名` → 通常幅
...
.venv/Scripts/python.exe docs/helper/setup_relume_project.py \
  --root-dir "s:/MyProjects/KAMUI_CODE/Relume/2025-10-08_satokupo-design-test/copy-of-ホームページ作成サービス" \
  --page "01_ホーム:hero-section:full-wide,service-features,pricing-table,contact" \
  --page "02_料金:pricing-hero,plan-comparison:wide,faq" \
  --page "03_サービス紹介:service-hero:full-wide,service-list,testimonials"
```

**注意**:
- セクション名リストにはヘッダーナビ・フッターを含めません
- 幅指定は`:full-wide`または`:wide`をセクション名の後ろに付けます
