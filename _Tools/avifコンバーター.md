# AVIF to PNG 変換ツール作成指示書

## 目的
複数のAVIFファイルをPNG形式に一括変換するGUIツールを作成します。

## 作成場所
**ユーザーが指定する保存場所に作成してください。**
ユーザーから保存場所のパスを確認してから作業を開始してください。

## 作成するファイル
`avif_to_png_converter.py`
...
## 依存関係のインストール
作成前に、仮想環境内で以下のコマンドを実行してPillowをインストールしてください：
...
以下の内容で `avif_to_png_converter.py` を作成してください：
...
作成後、以下のコマンドで実行してください：

```bash
python avif_to_png_converter.py
```

## 使い方

1. GUIウィンドウが起動します
2. 「ファイルを選択」ボタンをクリックして、変換したいAVIFファイルを複数選択
3. 「変換開始」ボタンをクリック
4. 変換が完了すると、元のファイルと同じフォルダにPNGファイルが保存されます
5. 必要に応じて「キャンセル」ボタンで処理を中断できます
...
- キャンセル機能
- 元の画像サイズを維持
- 変換後のファイルは元のファイルと同じフォルダに保存（拡張子のみ.pngに変更）

## 注意事項

- 変換後のPNGファイルは、元のAVIFファイルと同じディレクトリに保存されます
- 同名のPNGファイルが既に存在する場合は上書きされます
- 仮想環境内で実行してください
